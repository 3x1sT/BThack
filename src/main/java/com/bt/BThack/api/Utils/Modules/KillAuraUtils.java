...
public class KillAuraUtils implements Mc {
    ...
    public static EntityPlayer filterPlayers(String mode) {
        ...
        return mc.world.playerEntities.stream().filter(entityPlayer -> entityPlayer != mc.player && !isFriend(entityPlayer, mode) && !isTeammate(entityPlayer,mode) && !isAlly(entityPlayer, mode) && entityPlayer.isEntityAlive()).min(Comparator.comparing(entityPlayer ->
