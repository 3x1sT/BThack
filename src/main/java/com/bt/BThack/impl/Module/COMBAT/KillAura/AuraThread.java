...
public class AuraThread extends Thread implements Mc {
                ...
                if (entity != null) {
                    if (KillAuraUtils.canBeSeeTarget("KillAura", entity) && !entity.isDead && KillAuraUtils.isCurrentMob(entity)) {
                        switch (Module.getMode("KillAura", "AttackMode")) {
                            case "CoolDown":
                                KillAuraUtils.CoolDownAttack(entity, this, (int) Module.getSlider("KillAura", "Post Cooldown"));
                                break;
                            case "Spam":
    ...
    private void attack(EntityPlayer target) {
        if (target.getHealth() > 0) {
            switch (Module.getMode("KillAura", "AttackMode")) {
                case "CoolDown":
                    KillAuraUtils.CoolDownAttack(target, this, (int) Module.getSlider("KillAura", "Post Cooldown"));
                    break;
                case "Spam":
