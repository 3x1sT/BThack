...
public class NewElytraFlight extends Module {
        ...
        if (event.getPacket() instanceof SPacketPlayerPosLook) {
            ...
            if (NewElytraFlightReEquipElytraThread.needReTakeOff) {
                if (getCheckbox(this.name, "Auto TakeOff")) {
                    if (packet.x != mc.player.posX || packet.y != mc.player.posY || packet.z != mc.player.posZ) {
    ...
    private void takeoff(PlayerTravelEvent event) {
        ...
        boolean closeToGround = mc.player.posY <= WorldExtensions.getGroundPos(mc.world, mc.player).y + height && !wasInLiquid && !mc.isSingleplayer();
    ...
    private void stateUpdate(PlayerTravelEvent event) {
        ...
        if (mc.gameSettings.keyBindSneak.pressed && isFlying) {
            if (needReleaseShift) {
    ...
    private void landing(PlayerTravelEvent event) {
        if (mc.player.onGround) {
            ...
            BThack.instance.settingsManager.getModuleSettingByName(this.name, "Auto Landing").setValBoolean(false);
            return;
        } else if (WorldExtensions.isLiquidBelow(mc.world, mc.player)) {
            ...
            double groundY = WorldExtensions.getGroundPos(mc.world, mc.player).y;
