    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        for (Button button : buttons) {
            ...
            if (button.isMouseOnButton(mouseX, mouseY)) {
                activeButton = button;
                MusicStorage.playSound(MusicStorage.buttonClicked, mc.gameSettings.getSoundLevel(SoundCategory.MASTER) / 2);
            }
        }
        ...
        if (getButtonFromId(-1).isMouseOnButton(mouseX, mouseY)) {
            ...
            ActionBotConfig.tasks.add(taskButton.getId(), new MoveTask(xFrame.getNumber(), zFrame.getNumber(), scaffoldButton.getBVal(), moveType));
