...
public class LargeNicknames extends Module {
    ...
    public void onRender(RenderWorldLastEvent event) {
        ...
        for (RenderLivingEvent.Specials.Pre e : events) {
            ...
            if (AlliesUtils.isAlly((player.getDisplayNameString()))) {
                Ally ally = AlliesUtils.getAlly((player.getDisplayNameString()));
                assert ally != null;
                int color = new Color(ally.getR(), ally.getG(), ally.getB(), 1).hashCode();
                RenderUtils.drawNameplate(mc.fontRenderer, player.getDisplayName().getFormattedText() + ChatFormatting.WHITE +" HP:" + colour + health + ChatFormatting.RESET + " [" + ChatFormatting.BOLD + ally.getClanName() + ChatFormatting.RESET + "]", 0,0,0,0,
