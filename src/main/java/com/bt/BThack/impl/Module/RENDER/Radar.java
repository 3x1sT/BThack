...
import com.bt.BThack.api.Utils.Interfaces.Fr;
...
public class Radar extends Module implements Fr {
    ...
    public void onRender(RenderGameOverlayEvent.Post e) {
        if (Objects.requireNonNull(e.getType()) == RenderGameOverlayEvent.ElementType.TEXT) {
            ...
            FontUtils.drawCenteredString(fr, "X-", sr.getScaledWidth() - 6, sr.getScaledHeight() - ((int) scale / 2), -1);
            fr.drawString("X+", (int)(sr.getScaledWidth() - scale) + 2, sr.getScaledHeight() - ((int) scale / 2), -1);
            FontUtils.drawCenteredString(fr, "Z+", sr.getScaledWidth() - (int)(scale / 2), (sr.getScaledHeight() - (int) scale) + 1, -1);
            FontUtils.drawCenteredString(fr, "Z-", sr.getScaledWidth() - (int)(scale / 2), sr.getScaledHeight() - 9, -1);
                ...
                if (player != mc.player) {
                    ...
                    if (radarPosX < (int) range && radarPosZ < (int) range) {
                        ...
                        if (isCurrentCords(x,y, (sr.getScaledWidth() - (int) scale) + 1, sr.getScaledWidth() - 1, (sr.getScaledHeight() - (int) scale) + 1, sr.getScaledHeight() - 1)) {
                            ...
                            } else if (AlliesUtils.isAlly(player)){
                                Ally ally = AlliesUtils.getAlly(player.getDisplayNameString());
                                RenderUtils.drawSquare(x,y,3, -1);
                                assert ally != null;
                                Color color = new Color(ally.getR(), ally.getG(), ally.getB());
